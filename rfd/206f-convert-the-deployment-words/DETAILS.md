...
RFD 201c made every component a hexagon with a `core/` + `ports/` +
...
- Two names for one component cost more than the deployment words earn. A
  ...
  that the process holds a tick, and that the tick is what RFD 201c calls
...
logic" of RFD 201c, and a field on the wire in RFD 205b. The first becomes
...
The word "edge" had two meanings. RFD 201c said "concrete I/O at the edges",
and an edge plane was a process. RFD 201c now says "outside the interactor",
...
"repository" for a git repository, in RFD 2000, RFD 203e, RFD 203f, and
RFD 2040. That is a new word with two meanings, and this RFD makes it.
...
Three git repositories hold the RFD 201c triad on disk: `loot`, `combat`,
...
`fabric-flow-adapters` and its `-project` companion carry RFD 201c's
...
it comes from upstream, and it is not the "core" of RFD 201c. Renaming it
...
- This RFD does not rename the `domain` field on the wire in RFD 205b. That
