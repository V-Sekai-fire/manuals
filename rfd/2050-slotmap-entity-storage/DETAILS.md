...
- **RFD 200a** (binary encoding): FDB values are packed structs. The
  slotmap stores the same struct in memory — `memcpy` from FDB value
  to slotmap entry, no conversion.
- **RFD 2049** (async callbacks): FDB range scan callback populates
  the slotmap. Each KV pair in the range result is one
  `slotmap_insert`.
- **RFD 2010** (zstd): if the FDB value is compressed, decompress
