---
title: "RFD 2050: Slotmap entity storage with generational IDs"
rfd: "2050"
state: published
...
- Source: `weftspun/h2o-bench-tpcc`, `rfd/2011-slotmap-entity-storage.md`
- Consolidation target: `v-sekai-multiplayer-fabric/zone-server-h2o`

## Related

`rfd/204a-binary-value-encoding-for-fdb` (the format the slotmap
stores by `memcpy`), `rfd/204c-macaroon-xdp-security` (the
`entity_id` caveat maps to a slotmap handle), and
`rfd/2048-actor-lite-worker-pool` (each worker owns its zone's slotmap
thread-locally).
