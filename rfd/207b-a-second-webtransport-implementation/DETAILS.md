...
slow. Both repositories now set the buffer explicitly and keep safe overflow, because RFD 206c
...
sent. RFD 206c says a subscriber whose cursor falls out of the ring "receives a resync signal
...
Both C repositories carried a description using a word RFD 206f retired — "hands the result to a
...
RFD 206f retires names that say how a thing was built, and `-c` and `-python` are exactly that. The
...
`transport-picoquic`, which RFD 206f renamed from `fabric-edge`, is the precedent for naming a
