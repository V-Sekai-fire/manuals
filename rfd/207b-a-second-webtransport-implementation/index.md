---
title: "RFD 207b: A second WebTransport implementation, and the names that make room for it"
rfd: "207b"
state: discussion
...
and `contract-http3-queue`, and implements it once. RFD 2058 chose that: "Client and server share
...
one contract are distinguished only by how they are built, so RFD 206f's preference for names that
...
The cost is a fourth transport repository and a second wire reader to keep in step. RFD 207a named
...
- RFD 2058 chose one QUIC stack on both ends; RFD 207a rejected a second implementation on the
  player path and is not contradicted; RFD 202f abandoned `webtransportd` and still governs the
  production path; RFD 206f sets the name shape this amends for four repositories
- RFD 2031 for the reliability classes the two repositories split on, RFD 2060 for the measurement
  discipline, RFD 206c for the rule that a lagging reader never stalls the writer
