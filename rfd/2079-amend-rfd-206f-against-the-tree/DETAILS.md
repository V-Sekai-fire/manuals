...
The renames of RFD 206f have landed. GitHub answers the old names with the new
ones:
...
`^fabric` and is the name RFD 206f frees, so it is excluded here.
...
Five archived and four forks leave six live and owned. RFD 206f names
...
RFD 206f defends this name in one paragraph, and the paragraph argues a different
...
The test RFD 206f applies elsewhere is whether a name asserts something false.
...
RFD 206f renames the RFD 201c triad in three git repositories. Those three exist
...
RFD 206f states the resolution for the word it takes from Netflix: write "git
...
RFD 206f says the `lean-*-core` READMEs state a hexagon layout of `core/`,
...
So RFD 206f describes five READMEs where two exist. `entities-lean-shared` is the
...
The `domain` field on the wire in RFD 205b is still unrenamed, and still needs
its own RFD, as RFD 206f says.
