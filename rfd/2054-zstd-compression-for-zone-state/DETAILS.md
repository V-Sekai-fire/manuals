...
2. TPC-C transaction values: packed C structs (RFD 200a) are
...
## Relationship to RFD 200a (binary value encoding)

RFD 200a mandates packed C structs with `#pragma pack(push, 1)`. zstd
operates on the serialized byte stream, not the struct. The pipeline:

1. Pack struct to byte array (RFD 200a)
...
6. Cast byte array to struct (RFD 200a zero-copy)
