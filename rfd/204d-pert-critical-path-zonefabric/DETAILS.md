...
| --- | ----------------------------------------------- | ---------- | --- | --- | --- | --- | ---- |
| A   | Binary value encoding (RFD 200a)                | —          | 1   | 2   | 4   | 2.2 | 0.25 |
| B   | FDB keyspace + async callbacks (RFD 2009, 0011) | A          | 2   | 3   | 6   | 3.3 | 0.44 |
| C   | Actor-lite worker pool (RFD 2005)               | B          | 3   | 5   | 9   | 5.3 | 1.00 |
| D   | Slotmap entity storage (RFD 2011)               | A, B       | 2   | 3   | 5   | 3.2 | 0.25 |
| E   | Zstd compression (RFD 2010)                     | A          | 1   | 2   | 3   | 2.0 | 0.11 |
...
| L   | Macaroon + XDP security (RFD 2012)              | C, D       | 4   | 7   | 14  | 7.7 | 2.78 |
| M   | Feature ablation (RFD 2013)                     | F, H, I    | 3   | 5   | 8   | 5.2 | 0.69 |
...
- E (Zstd) has 11.1 days of slack. Compression can be added any
  time before zone-state blob persistence. This confirms RFD 2013's
  ablation result: zstd is deferrable. Do NOT build it first.

- L (Macaroon + XDP) has 7.5 days of slack. Security can be built
  in parallel with game logic. This confirms RFD 2013: security
...
- D (Slotmap) has only 2.1 days of slack and is near-critical.
  This confirms RFD 2013: slotmap is foundational and must be built
...
Based on the PERT analysis and RFD 2013 ablation matrix:
...
- **RFD 2013** (feature ablation): The dependency graph in this PERT
...
- **RFD 200e** (CI): The CI pipeline is assumed to exist from the
  start (building on every commit). It is infrastructure that
  supports all tasks, not a separate task.

- **RFD 200d** (benchmark harness): Task N is the wrk script
